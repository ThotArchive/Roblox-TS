...
const realtimeClient = function (sourceConstructors) {
  ...
  const onNotification = function (notificationSource, notification) {
    ...
    if (settings.isRealtimeWebAnalyticsEnabled) {
      try {
        ...
        events.maybeSendEventToDataLake(namespaceId, details, payloadSize);
      } catch (e) {
