...
function robloxImagesService($timeout, $log, httpService, retryService) {
    ...
    var retryTimer = imageRetryDataElment.length > 0 && Number(imageRetryDataElment.data("image-retry-timer")) || 1500;
    var httpFailureMsg = "Errors from http call: ";
    var maxTry = imageRetryDataElment.length > 0 && Number(imageRetryDataElment.data("image-retry-max-times")) || 10;
    ...
    function imageRetry(promiseData, image, callback, count, exponentialBackOff) {
        if (retryService.isExponentialBackOffEnabled) {
            ...
            if (count >= maxTry || promiseData.Final) {
                ...
                $timeout(function () { getImageUrl(promiseData, callback, count, exponentialBackOff); }, delay);
            }
        } else {
            ...
            if (count >= maxTry || promiseData.Final) {
                ...
                // pass into the retryUrl
                promiseData['RetryUrl'] = image.RetryUrl;
                $timeout(function () { getImageUrl(promiseData, callback, count); }, retryTimer);
            }
        }
        return false;
    }
    function getImageUrl(image, callback, count, exponentialBackOff) {
        ...
        httpService.httpGet(urlConfig, null)
                .then(
                    function (data) {
